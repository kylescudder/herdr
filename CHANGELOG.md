...
### Changed
- Synced with upstream herdr 0.8.0, bringing in all upstream changes since the fork point, including the GitHub organization migration to `herdrdev/herdr`. See the [0.8.0] section below for the full upstream list.

### Fork
- Preserves the fork-specific features carried since 0.7.5: worktree parent grouping (`parent_workspace_id` with the move-to-workspace picker and `workspace:` target selector), tmux-navigator smart pane focus, selection-copy coordinate clamping, workspace-picker deduplication and loose-worktree exclusion, and `j`/`k` navigation in the move-to-workspace picker.
