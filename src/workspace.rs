...
pub struct Workspace {
    ...
    /// Stable id of the workspace this one is explicitly filed under, if any.
    /// When set it wins over git-key grouping, so a worktree can nest under a
    /// chosen top-level workspace (monorepo-friendly). References `Workspace::id`.
