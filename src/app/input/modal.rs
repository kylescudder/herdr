...
pub(super) fn apply_context_menu_action(
    ...
    match (menu.kind, item) {
        ...
        (ContextMenuKind::GitWorkspace { ws_idx, .. }, Some("Move to workspace...")) => {
            state.request_move_workspace = Some(ws_idx);
            leave_modal(state);
        }
        (ContextMenuKind::Workspace { ws_idx }, Some("Move to workspace...")) => {
            state.request_move_workspace = Some(ws_idx);
            leave_modal(state);
        }
        (ContextMenuKind::GitWorkspace { ws_idx, .. }, Some("Collapse" | "Expand")) => {
            // Collapse groups by the group header's workspace id (the fork's
            // grouping model), matching the sidebar display and the chevron
            // click path, not by the git-repo key.
            if let Some((key, collapsed)) = crate::ui::workspace_parent_group_state(state, ws_idx) {
                if collapsed {
