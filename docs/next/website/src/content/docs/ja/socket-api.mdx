...
| ワークスペース | `workspace.create`、`workspace.list`、`workspace.get`、`workspace.focus`、`workspace.rename`、`workspace.move`、`workspace.move_block`、`workspace.report_metadata`、`workspace.close`、`workspace.reparent`、`workspace.acknowledge` |
...
`workspace.reparent` は `workspace_id` を `parent_workspace_id` の下に配置し、サイドバーでそのワークスペースの下に入れ子表示します。`parent_workspace_id` を省略するとトップレベルに戻します。自分自身の下への配置や、推測された git ワークツリーの親を含めて既に自分の子孫であるワークスペースの下への配置は拒否されます。レスポンスには確定したワークスペース一覧が含まれ、`workspace.updated` が発行されます。

`workspace.acknowledge` は `workspace_id` 内のすべてのペインを既読にし、完了した「done」のエージェントを再実行したりワークスペースを閉じたりせずに「idle」へ戻します。done マーカーは、エージェントが次に操作されるまで保持され、ペインやワークスペースにフォーカスしても消えません。確認すると、それらのペインの保留中の完了通知も取り消されます。
