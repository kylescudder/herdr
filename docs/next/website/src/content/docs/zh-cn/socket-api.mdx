...
| 会话 | `session.snapshot` |
| 工作区 | `workspace.create`、`workspace.list`、`workspace.get`、`workspace.focus`、`workspace.rename`、`workspace.move`、`workspace.move_block`、`workspace.report_metadata`、`workspace.close`、`workspace.reparent`、`workspace.acknowledge` |
...
`workspace.reparent` 会把 `workspace_id` 归到 `parent_workspace_id` 下面,侧边栏会将其嵌套显示在该工作区之下;省略 `parent_workspace_id` 则把它移回顶层。把工作区归到它自己下面,或归到已经是它后代的工作区下面(包括通过推断出的 git 工作树父级),都会被拒绝。响应包含服务器确认的工作区列表,并发出 `workspace.updated`。

`workspace.acknowledge` 会把 `workspace_id` 中的所有窗格标记为已读,让已完成的 “done” 代理回到 “idle”,既不会重新运行它,也不会关闭工作区。否则 done 标记会一直保留到该代理再次被使用为止,聚焦窗格或工作区并不会清除它。确认时还会取消这些窗格上待发送的完成通知。
