...
按 `prefix+g` 打开会话导航器:一棵可搜索的树,包含每个工作区、标签页和窗格。用 `j`/`k` 或方向键移动选择,按 `/` 搜索,按 `b/w/i/d/a` 按代理状态筛选,按空格折叠或展开工作区,按回车跳转到高亮行。

不离开键盘也能给项目排序:在未筛选的列表中选中某一行后,按 `shift+k` 把它的工作区上移,按 `shift+j` 下移,这样最常用的项目就能排在最前面,而不用管它们是什么时候打开的。工作树分组会作为一个整体移动。这相当于在侧边栏拖动工作区的键盘版本,同样的 `shift+k` / `shift+j` 排序在侧边栏工作区选择器(`prefix+w`)中也可以使用。

把一个工作区归到另一个工作区下面即可形成嵌套:在想移动的工作区上按 `prefix+shift+m`,然后用 `j`/`k` 选择目标并按回车,就会把它归到目标下面。源工作区连同它自己的工作树作为一个整体移动,并在所选父项下缩进显示,因此即使 git 无法推断出它们的关系(例如同一个 monorepo 中的多个工作区),你也可以把相关项目归到一起。在源工作区自身上按回车会把它移回顶层,按 Esc 则取消且不移动任何内容。同样的操作也可以从工作区的右键菜单(“Move to workspace”)中使用。
