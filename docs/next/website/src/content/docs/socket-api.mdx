...
| Workspace | `workspace.create`, `workspace.list`, `workspace.get`, `workspace.focus`, `workspace.rename`, `workspace.move`, `workspace.move_block`, `workspace.report_metadata`, `workspace.close`, `workspace.reparent`, `workspace.acknowledge` |
...
`workspace.reparent` files `workspace_id` under `parent_workspace_id`, so the sidebar nests it beneath that workspace; omit `parent_workspace_id` to return it to the top level. It rejects filing a workspace under itself or under one that already descends from it, including through an inferred git-worktree parent. The response contains the authoritative workspace list, and `workspace.updated` is emitted.

`workspace.acknowledge` marks every pane in `workspace_id` as seen, so a finished "done" agent returns to "idle" without re-running it or closing the workspace. A done marker otherwise persists until its agent is addressed again; focusing the pane or workspace does not clear it. Acknowledging also cancels any queued completion notification for those panes.
...
server and shows a client-local notice; it does not disconnect the UI. Rejected
