...
fn main() {
    ...
    // HERDR_SKIP_ZIG lets a prebuilt zig-out/lib archive be reused without
    // re-running `zig build` (used to build natively on hosts where the Zig
    // build runner is broken). Link directives below are still emitted.
    if std::env::var("HERDR_SKIP_ZIG").is_err() {
        let status = command
